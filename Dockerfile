...
RUN tar -xvf apache-tomcat-9.0.73.tar.gz
RUN mv apache-tomcat-9.0.73 /opt/
RUN cd /opt
RUN mkdir warfile/
COPY jpetstore.war /opt/warfile/
RUN mv /opt/warfile/jpetstore.war /opt/apache-tomcat-9.0.73/webapps/
RUN /opt/apache-tomcat-9.0.73/bin/shutdown.sh run
RUN cd /opt
RUN mkdir java-apm
...
ENTRYPOINT [ "/opt/apache-tomcat-9.0.73/bin/catalina.sh", "run" ]
